...
export const runScriptSteps = async (opts) => {
    const { logger, env, servers, defServer, vars, sessions, handlers, scriptOpts, stack, beforeStep, afterStep, } = opts;
    ...
    for (const step of steps) {
        ...
        if (step.delay) {
            logger.info(`${stepPrefix} waiting for delay=${step.delay} before starting step`);
        ...
        try {
                ...
                const query = step.request.query
                    ? "?" +
                        Object.entries(step.request.query)
                            ...
                            .join("&")
                    : "";
                const url = (rawUrl.includes("{{") ? evalTpl(rawUrl, ctx) : rawUrl) + query;
            ...
            /* ---------- call handlers ------------------------------------ */
            res = await runStepHandlers(step, handlers, logger, stepPrefix, cx, vars, sessions, res);
            ...
            step.response?.validate?.forEach((validation) => validation.check.forEach((expr) => {
                ...
                try {
                    ...
                    if (!pass) {
                        logger.warn(`${stepPrefix} FAILED expr=${expr} cx=\n${dumpContext(cx)}`);
                    }
                    overall &&= pass;
                    checkDetails.push({
                        name: validationId,
