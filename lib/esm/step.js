...
export const runScriptSteps = async (opts) => {
    ...
    for (const step of steps) {
        ...
        try {
            ...
            /* ---------- call handlers ------------------------------------ */
            if (step.handlers) {
                for (const [stepHandlerName, stepHandlerParams] of Object.entries(step.handlers)) {
                    ...
                    if (initHandler.args) {
                        for (const [field, config] of Object.entries(initHandler.args)) {
                            if (typeof stepHandlerParams[field] === "undefined") {
                                if (config.required) {
                                    throw new Error(`${stepPrefix} handler=${stepHandlerName} missing required arg=${field}`);
                                }
                    ...
                    const args = Object.fromEntries(Object.entries(stepHandlerParams).map(([param, value]) => [
                        param,
                        evalArgWithType(value, cx, initHandler.args ? initHandler.args[param].type : undefined, `${stepPrefix} handler=${stepHandlerName} wrong type for arg=${param}`),
                    ]));
