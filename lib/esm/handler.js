...
export const runStepHandlers = async (step, handlers, logger, stepPrefix, cx, vars, sessions, res) => {
    if (step.handlers) {
        for (const stepHandler of step.handlers) {
            ...
            const hDesc = `${hName}${stepHandler.comment ? `( ${stepHandler.comment})` : ""}`;
            if (stepHandler.delay) {
                logger.info(`${stepPrefix} waiting for delay=${stepHandler.delay} before starting ${hDesc}`);
            ...
            const args = Object.fromEntries(Object.entries(stepHandlerParams).map(([param, value]) => [
                ...
                initHandler.args && initHandler.args[param].opaque
                    ? value
                    : evalArgWithType(value, cx, initHandler.args ? initHandler.args[param].type : undefined, `${stepPrefix} handler=${hDesc} wrong type for arg=${param}`),
            ]));
            const varsForHandler = { ...vars, ...sessions };
            const origKeys = Object.keys(varsForHandler);
            logger.info(`${stepPrefix} invoking handler=${hDesc} with args=${JSON.stringify(args)}`);
