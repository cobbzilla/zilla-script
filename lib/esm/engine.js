    ...
    for (const step of script.steps) {
        ...
        try {
            ...
            if (step.request.session) {
                if (!srv.session) {
                    throw new Error(`step=${step.name ?? "?no step name"} session not supported by server=${srv.name}`);
                }
            ...
            step.response?.validate?.forEach((validation) => validation.check.forEach((expr) => {
                ...
                try {
                    ...
                    if (!pass) {
                        logger.debug(`FAILED step=${step.name} expr=${expr} cx=${JSON.stringify(cx)}`);
                    }
                    overall &&= pass;
                    checkDetails.push({
            ...
            if (!overall) {
                const msg = `validation failed in step ${step.name ?? "?"}: ${JSON.stringify(checkDetails, null, 2)}\n
