  ...
  for (const step of script.steps) {
    try {
      ...
      if (step.response?.session) {
        ...
        if (typeof tok === "string") sessions[step.response.session.name] = tok;
      }
      ...
      const details: { check: string; result: boolean }[] = [
        { check: "status", result: statusOk },
      ];
      let overall = statusOk;

      step.response?.validate?.forEach((val) =>
        val.check.forEach((c) => {
      ...
      stepResults.push({
        ...
        validation: {
          name: "error",
          result: false,
          details: [{ check: (err as Error).message, result: false }],
        },
        vars: { ...vars },
        sessions: { ...sessions },
      });
